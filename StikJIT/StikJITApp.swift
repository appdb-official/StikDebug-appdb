...
struct WelcomeSheetView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            Text(
                "Thanks for installing the app. This brief introduction will help you get started."
            )
            .font(.body)
            ...
            Text(
                "StikDebug is an on-device debugger designed specifically for self-developed apps. It helps streamline testing and troubleshooting without sending any data to external servers."
            )
            .font(.callout)
            .foregroundColor(.secondary)
            .multilineTextAlignment(.center)
            ...
            Text(
                "The next step will prompt you to allow VPN permissions. This is necessary for the app to function properly. The VPN configuration allows your device to securely connect to itself — nothing more. Rest assured, no data is collected or sent externally. Everything stays on your device."
            )
            .font(.callout)
            .foregroundColor(.secondary)
            .multilineTextAlignment(.center)
...
class TunnelManager: ObservableObject {
    ...
    private init() {
        ...
        NotificationCenter.default.addObserver(
            self, selector: #selector(statusDidChange(_:)), name: .NEVPNStatusDidChange, object: nil
        )
    }
    ...
    private func createOrUpdateTunnelConfiguration(completion: @escaping (Bool) -> Void) {
        NETunnelProviderManager.loadAllFromPreferences { [weak self] (managers, error) in
            ...
            manager.saveToPreferences { [weak self] error in
                ...
                DispatchQueue.main.async {
                    if let error = error {
                        VPNLogger.shared.log(
                            "Error saving tunnel configuration: \(error.localizedDescription)")
                        completion(false)
                        return
                    }
                    self.vpnManager = manager
            ...
            if wifiConnected {
                ...
                group.notify(queue: DispatchQueue.main) {
                    if self.controlIP == nil {
                        ...
                        self.dnsError =
                            "Apple DNS blocked. Your network might be filtering Apple traffic."
                        print("Control lookup succeeded, but Apple lookup failed: likely blocked.")
                    } else {
                        self.dnsError = nil
                        print(
                            "DNS lookups succeeded: Apple -> \(self.appleIP!), Control -> \(self.controlIP!)"
                        )
                    }
                }
            } else {
...
struct HeartbeatApp: App {
    ...
    func newVerCheck() {
        ...
        let VUA =
            UserDefaults.standard.object(forKey: "VersionUpdateAlert") as? Date ?? Date.distantPast
    ...
    private func isAppUpdateAvailable() {
        Appdb.shared.isAppUpdateAvailable { result in
            ...
            case let .success(value):
                DispatchQueue.main.async {
                    if value {
                        ...
                        self.alert_string =
                            "A new version is available on appdb. Would you like to update now?"
                        self.show_alert = true

                        // Store the update flag to handle the button action
                        self.showUpdateAlert = true
                    } else {
                        print("No app update available")
                    }
                }
            case let .failure(error):
                DispatchQueue.main.async {
                    ...
                    // self.alert_title = "Update Check Failed"
                    // self.alert_string = "Unable to check for updates: \(error.localizedDescription)"
                    // self.show_alert = true
                }
            }
        }
    ...
    private func openAppdbStore() {
        if case let .success(storeURL) = Appdb.shared.getAppdbStoreURL() {
    ...
    var body: some Scene {
        WindowGroup {
            Group {
                if isLoading2 {
                    LoadingView(
                        showAlert: $show_alert, alertTitle: $alert_title,
            ...
            if connection?.state != .ready {
                ...
                DispatchQueue.main.async {
                    if timeoutWorkItem?.isCancelled == false {
                        callback(
                            false,
                            "[TIMEOUT] The loopback VPN is not connected. Try closing this app, turn it off and back on."
                        )
                    }
                }
            }
            ...
            case .failed(let error):
                ...
                DispatchQueue.main.async {
                    if error == NWError.posix(.ETIMEDOUT) {
                        callback(
                            false,
                            "The loopback VPN is not connected. Try closing the app, turn it off and back on."
                        )
                    } else if error == NWError.posix(.ECONNREFUSED) {
                        callback(
                            false, "Wifi is not connected. StikJIT won't work on cellular data.")
                    } else {
        ...
        if isPairing(), !isMounted() {
            ...
            mountingThread = Thread {
                let mountResult = mountPersonalDDI(
                    ...
                    trustcachePath: URL.documentsDirectory.appendingPathComponent(
                        "DDI/Image.dmg.trustcache"
                    ).path,
                    manifestPath: URL.documentsDirectory.appendingPathComponent(
                        "DDI/BuildManifest.plist"
                    ).path,
                ...
                if mountResult != 0 {
                    showAlert(
                        title: "Error",
                        message:
                            "An Error Occured when Mounting the DDI\nError Code: \(mountResult)",
                        showOk: true, showTryAgain: true
                    ) { shouldTryAgain in
                        if shouldTryAgain {
                            self.mount()
                        }
                ...
                DispatchQueue.main.async {
                    ...
                    if result == -9 {
                        ...
                        showAlert(
                            ...
                            message:
                                "The pairing file is invalid or expired. Please select a new pairing file.",
                            showOk: true,
                            showTryAgain: false,
                            primaryButtonText: "Select New File"
                        ) { _ in
                            NotificationCenter.default.post(
                                name: NSNotification.Name("ShowPairingFilePicker"), object: nil)
                        }
                    } else {
                        showAlert(
                            title: "Heartbeat Error",
                            message:
                                "Failed to connect to Heartbeat (\(result)). Are you connected to WiFi or is Airplane Mode enabled? Cellular data isn't supported. Please launch the app at least once with WiFi enabled. After that, you can switch to cellular data to turn on the VPN, and once the VPN is active you can use Airplane Mode.",
                            showOk: false,
                            showTryAgain: true
                        ) { shouldTryAgain in
                            if shouldTryAgain {
            ...
            VStack {
                ZStack {
                    Circle()
                        .stroke(lineWidth: 8)
                        .foregroundColor(
                            isDarkMode ? Color.white.opacity(0.3) : Color.black.opacity(0.1)
                        )
...
func downloadFile(
    from urlString: String, to destinationURL: URL, completion: @escaping (String) -> Void
) {
    let fileManager = FileManager.default
