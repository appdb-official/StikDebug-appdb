...
struct HomeView: View {
    ...
    @State private var pairingFileExists = FileManager.default.fileExists(atPath: PairingFileStore.prepareURL().path)

    let timer = Timer.publish(every: 1, on: .main, in: .common).autoconnect()

    private var shouldShowAppdbImportCard: Bool {
        Appdb.shared.isInstalledViaAppdb() && !pairingFileExists
    }
    ...
    var body: some View {
        ...
        .overlay {
            if appdbImportManager.isImportingFile {
                ...
                ProgressView("Processing appdb pairing file…")
                    .padding(16)
                    .background(
    ...
    private func refreshPairingFileState() {
        pairingFileExists = FileManager.default.fileExists(atPath: PairingFileStore.prepareURL().path)
    }

    private var appdbImportCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            Label("Import from appdb", systemImage: "square.and.arrow.down.on.square")
                .font(.headline)
            Text("Download your pairing file directly from appdb services and refresh the device list automatically.")
                .font(.subheadline)
            ...
            Button {
                ...
                Label("Import from appdb", systemImage: "arrow.down.circle.fill")
                    .frame(maxWidth: .infinity)
            }
            .buttonStyle(.borderedProminent)
            .disabled(appdbImportManager.isImportingFromAppdb || appdbImportManager.isImportingFile)
        }
        .padding(14)
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
